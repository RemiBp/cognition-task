...
- **Narrow test coverage.** The suite covers denied actions and forged requests, invalid payloads, retries and conflicting payloads, self-approval, stale versions, concurrency, rollback on failure and the upgrade path. It is a policy suite, not production coverage: there is no end-to-end, load, or failure-injection testing of the surrounding system, and the concurrency tests run on SQLite, which serialises writes and therefore does not exercise what Postgres would.
- **Transaction boundaries.** The claim on a proposal, the business mutation and the success audit entry now commit as one local transaction, and denials and failures are written outside it so they survive a rollback. That is local atomicity only. Anything beyond this database, a payment processor above all, is simulated: a committed transaction here is not evidence that money moved.
